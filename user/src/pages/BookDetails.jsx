...
import { FiArrowLeft, FiBookmark, FiDownload, FiBookOpen, FiLoader, FiLock } from 'react-icons/fi';
...
export default function BookDetails() {
  ...
  // --- NEW: State for tracking the request button status ---
  ...
  useEffect(() => {
    const fetchBookData = async () => {
      ...
      try {
        ...
        console.log('User library could not be loaded or user is not logged in.');
      }
  ...
  const handleDownload = async (e) => {
    ...
    try {
      try {
        ...
        alert(`Database Log Error: ${logError.response?.data?.message || logError.message}`);
      ...
      alert("Failed to download the file. It might be missing from the server.");
    } finally {
      setIsDownloading(false);
    }
  };

  // --- UPDATED: Send the Request as a Chat Message ---
  ...
  if (loading) return <div className="flex justify-center py-32 animate-pulse text-gray-500 dark:text-slate-500">Loading book details...</div>;
  if (!book) return <div className="text-center py-32"><h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">Book not found</h2><button onClick={() => navigate(-1)} className="text-blue-600 dark:text-blue-500 hover:underline">Go Back</button></div>;

  const isSaved = Array.isArray(savedBookIds) && savedBookIds.includes(book._id);
  ...
  return (
    <div className="max-w-6xl mx-auto pb-12">
      <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white transition-colors mb-8">
        ...
        <div className="w-full md:w-1/3 lg:w-1/4 shrink-0">
          <div className="aspect-[2/3] bg-gray-100 dark:bg-slate-800 rounded-2xl shadow-xl overflow-hidden border border-gray-200 dark:border-slate-700">
            {book.cover_image ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-gray-400 dark:text-slate-600">No Image</div>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col">
          {book.genre && Array.isArray(book.genre) && book.genre.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {book.genre.map(g => (
                <span key={g} className="px-3 py-1 text-xs uppercase tracking-wider font-bold bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-full border border-blue-200 dark:border-blue-800/50">
                  {g}
                </span>
              ))}
            </div>
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-3 mb-10">
            ...
            {/* --- UPDATED: Direct API Request Button --- */}
            {isOldBook ? (
               <button 
               ...
               >
                 <FiLock className="text-lg" /> {requestStatus === 'pending' ? 'Request Sent!' : 'Request Access'}
               </button>
            ) : book.download_link ? (
              <>
                <Link 
                ...
                >
                  {isDownloading ? (
                    <><FiLoader className="text-lg animate-spin" /> Downloading...</>
                  ) : (
            ...
            >
              <FiBookmark className={isSaved ? 'fill-emerald-500 dark:fill-emerald-400' : ''} /> 
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-6 gap-x-4 py-6 border-y border-gray-200 dark:border-slate-800 text-sm transition-colors">
            <div><div className="text-gray-500 dark:text-slate-500 text-xs uppercase tracking-wider mb-1 font-semibold">Author</div><div className="text-gray-900 dark:text-slate-200">{book.author || '-'}</div></div>
            <div><div className="text-gray-500 dark:text-slate-500 text-xs uppercase tracking-wider mb-1 font-semibold">Publisher</div><div className="text-gray-900 dark:text-slate-200">{book.publisher || '-'}</div></div>
            <div><div className="text-gray-500 dark:text-slate-500 text-xs uppercase tracking-wider mb-1 font-semibold">Published</div><div className="text-gray-900 dark:text-slate-200">{book.publish_date || '-'}</div></div>
            <div><div className="text-gray-500 dark:text-slate-500 text-xs uppercase tracking-wider mb-1 font-semibold">ISBN</div><div className="text-gray-900 dark:text-slate-200">{book.isbn || '-'}</div></div>
            <div><div className="text-gray-500 dark:text-slate-500 text-xs uppercase tracking-wider mb-1 font-semibold">Format</div><div className="text-gray-900 dark:text-slate-200">{book.format || '-'}</div></div>
            <div><div className="text-gray-500 dark:text-slate-500 text-xs uppercase tracking-wider mb-1 font-semibold">Language</div><div className="text-gray-900 dark:text-slate-200">{book.language || '-'}</div></div>
            <div><div className="text-gray-500 dark:text-slate-500 text-xs uppercase tracking-wider mb-1 font-semibold">Size</div><div className="text-gray-900 dark:text-slate-200">{book.size || '-'}</div></div>
          </div>
          
          <div className="mt-8">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3 uppercase tracking-wider">Description</h3>
      ...
      {relatedBooks.length > 0 && (
        <div className="pt-12 border-t border-gray-200 dark:border-slate-800 transition-colors">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">More like this</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {relatedBooks.map((relatedBook) => {
              const isRelSaved = Array.isArray(savedBookIds) && savedBookIds.includes(relatedBook._id);
              return (
                <Link 
                ...
                >
                  <div className="relative aspect-[2/3] bg-gray-100 dark:bg-slate-800 overflow-hidden">
                    {relatedBook.cover_image ? (
                      <img src={relatedBook.cover_image} alt={relatedBook.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    ) : (
                    ...
                    >
                      <FiBookmark className={`text-sm ${isRelSaved ? 'fill-blue-500 text-blue-500' : 'text-gray-700 dark:text-white'}`} />
                    </button>
                  </div>
                  <div className="p-3 flex flex-col flex-1">
                    ...
                    <p className="text-[10px] md:text-xs text-gray-500 dark:text-slate-400 line-clamp-1">{relatedBook.author || 'Unknown Author'}</p>
                  </div>
                </Link>
              );
            })}
