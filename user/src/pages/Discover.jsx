...
const BookCard = ({ book, isSaved, onToggleBookmark, navigate }) => {
  ...
  const isRestricted =
    new Date(book.createdAt) < fiveYearsAgo && !book.unrestricted;
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative aspect-[2/3] bg-gray-100 dark:bg-slate-800 overflow-hidden ${isRestricted ? "sepia-[.15]" : ""}`}
      >
        {book.cover_image ? (
          <img
        ...
        >
          <FiBookmark
            className={`text-sm md:text-base ${isSaved ? (isRestricted ? "fill-orange-500 text-orange-500" : "fill-blue-500 text-blue-500") : "text-gray-700 dark:text-gray-200"}`}
          />
        </button>
        ...
        <div className="absolute inset-0 bg-slate-900/85 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-4 z-20 backdrop-blur-[2px]">
          <div>
            ...
            <p className="text-slate-300 text-[10px] md:text-xs line-clamp-4 leading-relaxed mt-2">
              {book.description ||
                `Explore ${book.title} by ${book.author || "this author"}. Click to view full details and access the document.`}
            </p>
          </div>
      ...
      <div className="p-3 flex flex-col flex-1 bg-white dark:bg-[#1e293b] z-10">
        <h3
          className={`font-bold text-gray-900 dark:text-gray-100 text-xs md:text-sm line-clamp-2 mb-1 transition-colors leading-snug ${isRestricted ? "group-hover:text-orange-600 dark:group-hover:text-orange-400" : "group-hover:text-blue-600 dark:group-hover:text-blue-400"}`}
        >
          {book.title}
        </h3>
        <p className="text-[10px] md:text-xs text-gray-500 dark:text-slate-400 mb-3 line-clamp-1">
...
export default function Discover() {
  ...
  const getPageSubtitle = () => {
    if (ageFilter === "old")
      return "Preserved historical books from 5+ years ago";
    if (searchQuery || currentCategory !== "All Books")
      return `Explore books in ${currentCategory}`;
  ...
  return (
    <div className="max-w-7xl mx-auto pb-12">
      ...
      {loading ? (
        <div className="flex justify-center py-20 animate-pulse text-gray-500 dark:text-slate-500 font-medium">
          Loading library...
        </div>
      ) : books.length === 0 ? (
