...
router.put("/profile", verifyToken, async (req, res) => {
  try {
    ...
    console.log(
      "2. Password extracted by backend:",
      incomingPassword ? "[HIDDEN - RECEIVED]" : "EMPTY/NULL",
    );
    ...
    if (incomingPassword && incomingPassword.trim() !== "") {
      ...
      console.log(
        "4. SKIPPED password hashing because incomingPassword was empty.",
      );
    }
...
router.post("/library/:bookId", verifyToken, async (req, res) => {
  try {
    ...
    await user.save();
    res.json({
      message: isSaved ? "Removed from library" : "Added to library",
