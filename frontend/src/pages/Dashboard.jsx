...
// Format big numbers (e.g., 45200 -> "45.2K")
...
export default function Dashboard() {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const [systemRes, booksRes] = await Promise.all([
          axios.get('http://localhost:5000/api/system/dashboard-stats', config).catch(() => ({ data: {} })),
          axios.get('http://localhost:5000/api/books?limit=5', config).catch(() => ({ data: { data: [] } }))
        ]);
  ...
  if (loading) {
    return <div className="p-6 text-slate-500 dark:text-slate-400 animate-pulse">Loading system overview...</div>;
  }
    ...
    <div className="p-6 text-slate-900 dark:text-slate-100 overflow-y-auto custom-scrollbar h-full bg-slate-100 dark:bg-[#0f172a]">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Real-time statistics and backend analytics.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <StatCard title="Total Books" value={data.stats.totalBooks} sub="Live Inventory" icon={<FiBarChart2 />} color="blue" />
        <StatCard title="Active Users" value={data.stats.activeUsers} sub="Registered Accounts" icon={<FiUsers />} color="indigo" />
            ...
            <table className="w-full text-left border-collapse min-w-max">
              ...
              <tbody>
                {data.recentBooks.map((book) => (
                  <tr key={book._id} className="border-b border-slate-100 dark:border-slate-800/40 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="p-4 flex gap-4 items-center border-none">
                      <div className="w-10 h-14 bg-slate-100 dark:bg-slate-800 rounded shadow-sm overflow-hidden flex-shrink-0 flex items-center justify-center border border-slate-200 dark:border-slate-700">
                        {book.cover_image ? (
                          ...
                          <span className="text-[9px] text-slate-400 text-center leading-tight p-1">No<br/>Cover</span>
                        )}
                      </div>
                      <div>
                        <div className="font-semibold text-sm text-slate-900 dark:text-slate-100 line-clamp-1">{book.title}</div>
                        <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-1">{book.author}</div>
                      </div>
                    </td>
                ...
                {data.recentBooks.length === 0 && (
                  <tr><td colSpan="4" className="p-8 text-center text-sm text-slate-500">No books found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="bg-white dark:bg-[#1e293b] rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-5 flex flex-col h-full">
          ...
          <div className="relative border-l-2 border-slate-200 dark:border-slate-700 ml-3 space-y-6 flex-1 mb-4">
            {data.logs.length > 0 ? (
              ...
              <p className="text-sm text-slate-500 pl-4">No system logs recorded yet.</p>
            )}
          </div>
...
function StatCard({ title, value, sub, icon, color }) {
  ...
  return (
    <div className="bg-white dark:bg-[#1e293b] p-5 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
      ...
      <div className={`text-sm mt-2 font-medium ${sub.includes('~') ? 'text-green-500' : 'text-slate-500 dark:text-slate-400'}`}>
        {sub}
      </div>
    </div>
  );
...
function LogItem({ type, title, desc, time }) {
  ...
  return (
    <div className="relative pl-6">
      <span className={`absolute -left-[5px] top-1.5 w-2 h-2 rounded-full ring-4 ${typeColors[type] || typeColors.info}`}></span>
      <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">{title}</h3>
      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-2">{desc}</p>
      <span className="text-xs text-slate-400 dark:text-slate-500 mt-1 block">{time}</span>
    </div>
  );
}