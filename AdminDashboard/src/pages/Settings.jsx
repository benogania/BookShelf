...
export default function Settings() {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    if (!formData.username && !formData.newPassword) {
      return setStatus({
        type: "error",
        message: "Please enter a new username or password to update.",
      });
    }
    ...
    try {
      ...
      if (!adminToken) {
        ...
        return setStatus({
          type: "error",
          message:
            "Authentication error: No admin token found. Please log out and back in.",
        });
      }
          ...
          <div className="bg-white dark:bg-[#1e293b] rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden">
            <div className="px-6 py-5 border-b border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-800/30">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Update your login username or password. Leave blank if you do
            ...
            <div className="p-6">
              {status.message && (
                <div
                  className={`mb-6 p-3 rounded-lg text-sm border ${status.type === "error" ? "bg-red-50 text-red-600 border-red-200 dark:bg-red-900/20 dark:border-red-800/50" : "bg-green-50 text-green-700 border-green-200 dark:bg-green-900/20 dark:border-green-800/50"}`}
                >
                  {status.message}
                </div>
              )}
          ...
          <div className="bg-white dark:bg-[#1e293b] rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden">
            ...
            <div className="p-6">
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">
                End your current session. You will be required to sign back in
