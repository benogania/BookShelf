...
export default function Downloads() {
    ...
    const fetchLogs = async () => {
      try {
        ...
        const res = await axios.get(
          "http://localhost:5000/api/books/logs/downloads",
          {
  ...
  const SortIcon = ({ columnKey }) => {
    if (sortConfig.key !== columnKey)
      return <FiChevronDown className="opacity-20 ml-1" />;
              ...
              <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">
                ...
                >
                  <div className="flex items-center">
                    Book Title <SortIcon columnKey="bookTitle" />
                  </div>
                </th>
                ...
                >
                  <div className="flex items-center">
                    Downloaded By <SortIcon columnKey="userName" />
                  </div>
                </th>
                ...
                >
                  <div className="flex items-center">
                    Date / Time <SortIcon columnKey="downloadedAt" />
                  </div>
                </th>
              </tr>
            </thead>
                    ...
                    <td className="p-4">
                      <div className="flex flex-col">
                        <span className="text-sm text-slate-700 dark:text-slate-300">
                          {new Date(
                            log.downloadedAt || log.createdAt,
                          ).toLocaleDateString()}
                        </span>
