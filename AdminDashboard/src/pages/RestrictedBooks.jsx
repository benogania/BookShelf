...
import { FiLock, FiUnlock, FiShield, FiSearch } from "react-icons/fi";
  ...
  const fetchRestrictedBooks = async () => {
    try {
      ...
      const res = await axios.get(
        "http://localhost:5000/api/books/admin/restricted",
        {
  ...
  const handleToggle = async (bookId, currentAction) => {
    try {
      const token =
        localStorage.getItem("adminToken") || localStorage.getItem("token");
      await axios.put(
        `http://localhost:5000/api/books/admin/restrict-toggle/${bookId}`,
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Manage your 5-year archives. Unrestrict books to make them public,
            or restrict them back to the vault.
          </p>
        </div>
              ...
              {filteredBooks.length === 0 ? (
                <tr>
                  <td colSpan="4" className="p-8 text-center text-slate-500">
                    {searchQuery
                      ? `No restricted books found matching "${searchQuery}".`
                      : "No restricted books found in the system."}
                  </td>
                </tr>
              ) : (
                filteredBooks.map((book) => {
                  ...
                  const isCurrentlyRestricted =
                    new Date(book.createdAt) < fiveYearsAgo;
                  ...
                  return (
                    ...
                    >
                      <td className="p-4 flex gap-3 items-center">
                        ...
                        <div>
                          <div
                            className="font-bold text-slate-900 dark:text-slate-200 text-sm line-clamp-1"
                            title={book.title}
                          >
