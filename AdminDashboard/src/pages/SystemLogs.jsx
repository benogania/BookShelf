...
export default function SystemLogs() {
    ...
    const fetchHistory = async () => {
      try {
        ...
        const res = await axios.get(
          "http://localhost:5000/api/system/logs/history",
          {
  ...
  const getLogColor = (type) => {
    switch (type) {
      ...
      case "warning":
        return "bg-yellow-500 ring-yellow-100 dark:ring-yellow-900/30";
      case "error":
          ...
          {filteredLogs.length === 0 ? (
            ...
            filteredLogs.map((log) => (
              <div key={log._id} className="relative pl-8 group">
                {/* Dot */}
                <span
                  className={`absolute -left-[5px] top-1.5 w-2 h-2 rounded-full ring-4 ${getLogColor(log.type)}`}
                ></span>
